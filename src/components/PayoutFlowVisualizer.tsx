...
import {
  Building2, Globe, Shield, FileCheck, Wallet, 
  CreditCard, Server, CheckCircle2, Clock, ArrowRight, 
...
const INITIAL_STEPS: PayoutStep[] = [
  {
    id: "merchant",
    name: "Merchant Initiates",
    icon: () => <Building2 className="w-5 h-5" />,
    status: "pending",
    latency: 0,
    description: "Payout request from merchant dashboard"
  },
  {
    id: "gateway",
    name: "API Gateway",
  ...
  },
  {
    ...
    status: "pending",
    latency: 10,
    description: "Merchant authentication and permissions"
  },
  {
    id: "compliance",
    name: "KYC/Compliance",
    ...
    status: "pending",
    latency: 65,
    description: "Regulatory checks and AML screening"
  },
  {
    id: "balance",
    name: "Balance Check",
    ...
    status: "pending",
    latency: 15,
    description: "Verify sufficient funds and holds"
  },
  {
    id: "payout",
    name: "Payout Service",
    ...
    status: "pending",
    latency: 20,
    description: "Payout transaction creation"
  },
  {
    id: "provider",
    ...
    status: "pending",
    latency: 420,
    description: "Bank/Provider transfer processing"
  },
  {
    id: "settlement",
    name: "Confirmation",
    ...
    status: "pending",
    latency: 30,
    description: "Settlement confirmation and notification"
  }
];
...
export const PayoutFlowVisualizer = () => {
  const [activeFlows, setActiveFlows] = useState<PayoutFlow[]>([]);
  const [totalProcessed, setTotalProcessed] = useState(0);
  const [successRate, setSuccessRate] = useState(100);

  const startNewPayout = () => {
    const newFlow: PayoutFlow = {
      id: `payout-${Date.now()}`,
      ...
      setActiveFlows(prev => {
        return prev.map(flow => {
          ...
          const updatedSteps = flow.steps.map((step, idx) => {
            ...
            if (idx === flow.currentStep) {
              // Simulate occasional failures at compliance check
              if (step.id === "compliance" && Math.random() < 0.08) {
                return { ...step, status: "failed" as const };
              }
              // Simulate occasional failures at provider
              if (step.id === "provider" && Math.random() < 0.05) {
                return { ...step, status: "failed" as const };
              }
          ...
          if (hasElapsed) {
            ...
            if (isFailed) {
              ...
              setSuccessRate(p => Math.max(88, p - 0.3));
  ...
  const getStatusColor = (status: PayoutStep["status"]) => {
    switch (status) {
  ...
  const getTotalLatency = (steps: PayoutStep[], upToStep: number) => {
    ...
    <div className="w-full max-w-7xl mx-auto p-6 space-y-6">
      <div className="text-center space-y-2">
        <h2 className="text-3xl font-bold bg-gradient-to-r from-primary via-secondary to-accent bg-clip-text text-transparent">
          Real-Time Payout Flow Visualization
        </h2>
        <p className="text-muted-foreground">Watch payouts flow through the microservices architecture</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4">
          ...
          <div className="text-xs text-muted-foreground">Active Payouts</div>
        </Card>
        <Card className="p-4">
      ...
      <div className="flex justify-center">
        <Button onClick={startNewPayout} className="gap-2">
          <Banknote className="w-4 h-4" />
          Simulate New Payout
        </Button>
      </div>

      {/* Payout Flow Steps Reference */}
      <Card className="p-6">
      ...
      <div className="space-y-4">
        ...
        {activeFlows.length === 0 && (
          <Card className="p-8 text-center text-muted-foreground">
            No active payouts. Click "Simulate New Payout" to start.
          </Card>
        )}
